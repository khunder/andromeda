...
export class ConnectionInsertHandler {
  private highlightedConnection: string | null = null;
  private highlightElement: SVGElement | null = null;
  private isDraggingOverConnection = false;
   ...
   */
  checkConnectionProximity(point: Point, elementType: string): string | null {
    const connections = this.connectionManager.getAllConnections();
    const threshold = 15; // Distance threshold in pixels
    
    for (const connection of connections) {
      if (connection.waypoints.length < 2) continue;
   ...
   */
  private canInsertBetween(source: BPMNElement, elementType: string, target: BPMNElement): boolean {
  ...
  highlightConnection(connectionId: string): void {
    if (this.highlightedConnection === connectionId) return;
   ...
   */
  insertNodeOnConnection(connectionId: string, elementType: string, dropPoint: Point): string | null {
    const connection = this.connectionManager.getConnection(connectionId);
   ...
   */
  getHighlightedConnection(): string | null {
